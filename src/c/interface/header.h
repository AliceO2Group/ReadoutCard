...
uint32_t
  DMA_PAGE_LENGTH,// Page length in bytes.
  ...
  MAX_EVENT,      // Maximum number of events generated by the card's internal data generator.
...
typedef struct CardStruct{
  ...
  int serialNumber;                   ///< Serial number of the card.
  int revisionNumber;           ///< Revision number of the card.
...
/// Initialize a card.
/// \return  Pointer to the initialized card struct.
Card* createCard();

/// Frees up the memory allocated for the Card object.
/// \param card Pointer to the Card obbject that is to be deleted.
/// \return     0 if the deallocaion was successful.
...
/// Gets the serial number of the card from the flash memory.
/// \param card     The card we want to access. 
/// \param channel  The channel of the buffer.
/// \return         The serial number.
int (*getSerialNumber)(Card *card, int channel);
